...
module.exports = {
    ...
    wts: function (message, command) {
        ...
        if (!btc || !token) {
            message.channel.send("otc wts [btc/token] [token]")
            return;
        }
        if (!/^[0-9\.]+$/.test(btc) || !/^[0-9\.]+$/.test(token)) {
        ...
        if (btc > 1 || btc <= 0.000000001) {
            message.channel.send("BTC price per " + token_symbol + " is either too high or too low. Range 0.00000001 - 1");
            return;
        }
        if (token <= 1) {
        ...
        message.channel.send("Market WTS order at " + btc + " for " + token + " " + token_symbol + " for a total of (" + total + " BTC)");
    ...
    wtb: function (message, command) {
        ...
        if (!btc || !token) {
            message.channel.send("otc wtb [btc/token] [token]")
            return;
        }
        if (!/^[0-9\.]+$/.test(btc) || !/^[0-9\.]+$/.test(token)) {
            message.channel.send("That's not a number I understand");
        ...
        if (token <= 1) {
            message.channel.send("Can't buy less than 1 " + token_symbol + " at a time");
            return;
        }
        var total = btc * token;
        ...
        message.channel.send("Market WTB order at " + btc + " for " + token + " " + token_symbol + " for a total of (" + total + " BTC)");
        ...
        try {
            ...
            if (wts.length > 0) {
                ...
                wts_sorted.forEach(function (arr) {
                    ...
                    output_in += btc.toFixed(8) + ' btc/' + token_symbol;
                    output_in += String(arr.token).padStart(7, " ") + " " + token_symbol + " ";
